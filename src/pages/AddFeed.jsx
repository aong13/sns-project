...
const AddFeed = ({ navigation }) => {
  const [uploadingImage, setUploadingImage] = useState(receivedImages);
  const [keywords, setKeywords] = useState([]);
  const [keyword, setKeyword] = useState('');
  const [content, setContent] = useState('');

  const handleDelete = (id) => {
    const updatedData = uploadingImage.filter(item => item.id !== id);
      ...
      <View style={{ margin: 16, width: width - 32 }}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imageWrapper}>
          {uploadingImage.map(item => (
            <View style={{ paddingHorizontal: 4 }} key={item.id}>
